...
input_B = 'right/';
% ---------------
PointsPerFrame = 1000; % # of feature correspondences in a frame
...
Smoothness = 4;                      % adjust how stable the output is, 1 - 4 is OK
Cropping = 1;                        % adjust how similar the result to the original video, usually set to 1;
Stitchness = 30;                     % adjust the weight of stitching term, 10 - 30 is OK
SKIP_BACKGROUND_SEGMENTATION = false; % skip the background segmentation - treat all tracks as background.
RANSAC = true;                       % use RANSAC to remove wrong sparse correspondences, set to true if overlap is small and stitch fails. 
% ---------------
OutputPadding = 700; % the padding around the video
