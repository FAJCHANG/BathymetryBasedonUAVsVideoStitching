classdef VideoStitch2 < handle
    ...
    methods
        function obj = VideoStitch2(seqA, seqB, pathA, pathB, ControlPoints, ppf, smoothness, cropping, stitchness)
            ...
            obj.smoothness = smoothness;%3
            obj.cropping = cropping;%1
            obj.stitchness = stitchness;%20
            obj.stableW = 20;
            obj.span = 30;
            obj.H = eye(3);
            obj.Offset = eye(3);
			...
			obj.nFrames = min(length(fileListA), length(fileListB));
			if obj.nFrames < 2
				error('Wrong inputs directory') ;
			end
            frame = imread([seqB fileListB(1).name]);

            [obj.videoHeight, obj.videoWidth, ~] = size(frame);
            [~, obj.meshSize, ~, ~, ~] = size(pathA);
            [~, obj.meshSize, ~, ~, ~] = size(pathB);
            [obj.nFrames, obj.maxppf, ~] = size(obj.CP);%后面两个忽略掉的是1，2，3，4分别是A和B的控制点坐标

            % convert CP to CPgrid
            obj.CPgrid = zeros(obj.nFrames, obj.meshSize, obj.meshSize, obj.maxppf / 10, 2);%为什么要除10？
...
%             offset = eye(3);
        ...
        function optPath(obj, maxIte, secondPhase)%15, 5
            % zero round
            firstround = maxIte - secondPhase;%10
            % parameters
