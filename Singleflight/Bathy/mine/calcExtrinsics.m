function info = calcExtrinsics(step4)
...
%     end
    ...
    inputDir = step4.inputDir;%输入文件路径
    intrinsic = load(step4.intrinsic);
    intrinsics = intrinsic.intrinsics;%内参
    
    gcpUV = step4.gcpUV;%第一帧控制点坐标
    Expara = step4.Expara;%第一帧外参
    path = step4.path;%视频路径
    ...
    L = string(ls(inputDir));
    L = L(3:end);
    for nf = 1:nFrames
        if nf == 1
            figure(1);
            I = imread(strcat(inputDir, L(nf)));
            imshow(I)
            hold on;
            for k=1:length(gcpUV)
                h1=plot(gcpUV(k).UVd(1),gcpUV(k).UVd(2),'ro','markersize',10,'linewidth',3);
            end
            saveas(gcf, [savepath 'gcpsPos/' num2str(nf, '%04d')], 'jpg');
            continue;
        end
        I = imread(strcat(inputDir, L(nf)));
