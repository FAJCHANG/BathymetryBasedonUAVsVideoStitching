...
%   此处显示详细说明
    ...
    gaussian_kernel = step.gaussian_kernel;
    ...
    idxdy = step.pixel_resolution;%分辨率
    ixlim = [0,250];%跨海长度
    iylim = [0,100];%海滩长度
    iz = 0;
    
    inputStruct.roi_x = [0,250];
    ...
    Lo = string(ls(inputdir));
    Lo = Lo(3:end);
    Lf = string(ls(filt_path));
    Lf = Lf(3:end);
    ...
    Z=X*.0+iz; 
    save([savepath '/GRID_roiInfo' ], 'X', 'Y', 'Z', 'worldCoord', 'localAngle','localOrigin','localX','localY','localZ');
...
%             temp_intrinsics = intrinsics;
            ...
            imagepath = strcat(inputdir, Lo(id));
            Io = imread(imagepath);
            fimagepath = strcat(filt_path, Lf(id));
            If = imread(fimagepath);
%             If = gaussfilter(Io,gaussian_kernel);
...
%                 pixImg(p).Irgb = Iorgb;
                [Ifgray]= imageRectifier(If, intrinsics,the_extrinsic,pixInst(p).X,pixInst(p).Y,pixInst(p).Z,0); 
...
%                 imshow(Ifgray);
        ...
        save([savepath 'pixelImg_' num2str(gaussian_kernel)], 'pixelImg');
            
    else
        disp('图片数量与控制点外参项数目不一样');
    end
