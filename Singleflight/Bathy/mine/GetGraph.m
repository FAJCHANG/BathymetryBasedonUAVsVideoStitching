function [Path, graph, goodA, goodB] = GetGraph( trackA, trackB, CP, ppf, alpha, beta)
                ...
                for video = 1:2
                    ...
                    for u = 1:t_nLabel1
                        for v = 1:t_nLabel2                        
                            ...
                            % compute the trajectory matrix 计算轨迹矩阵
    ...
    %         fprintf('intra-window\n=>');
            % 1. scan all the frames inside the window.  扫描窗口内的所有帧。
            left = (windowIndex - 1) * wSize / 2 + 1;
            right= (windowIndex + 1) * wSize / 2; 
            edges = zeros(nLabel);
...
%             Mv = zeros(wSize, nLabel);
            ...
            for fIndex = 1:wSize
                ...
                pb = squeeze(round(CP(frameIndex, 1:ppf(frameIndex), 1:2)));
