classdef Graph < handle
    ...
    methods
         function obj = Graph(windowNum, clusterNum, faj)
             if nargin > 3
                 error('Graph construcor too many inputs, require at most 3 optional inputs');
             end
             ...
             switch nargin
                 ...
                 case 3
                     obj.windowNum = windowNum;
                     obj.clusterNum = clusterNum;%按照这样写的话，clusterNum就要达到A或B的nlabel那么大了。
             end
             
             obj.maxDistance = 1e10; 
             obj.dftDistance = 1e6;    % 1e8
                          
             obj.nodeNum = faj * obj.clusterNum * obj.windowNum;%节点数量，这个2应该是两个视频的意思
             ...
             % be very large
             obj.adjacentMatrix = ones(obj.nodeNum, obj.nodeNum) * obj.dftDistance;%设定邻接矩阵
         ...
         function Path = getShortestPath(obj)
            ...
            % window to the last window for video 1    从视频1的第一个窗口到最后一个窗口的顶点索引列表
...
%                  disp(caseCode);
        ...
        function computeShortestPathForVideo1From0tok(obj)%代码！！！
        ...
        function computeShortestPathForVideo1Fromkto0(obj)%从k到0
            k = obj.windowNum;
            node1VideoIdx = 0;
