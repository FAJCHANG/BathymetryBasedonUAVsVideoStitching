classdef TrackLib < handle
    ...
    methods
        ...
        function addPoints(obj, newPoints, frameIndex)
            ...
            obj.points(obj.nTrack + 1:obj.nTrack + nNew, frameIndex, :) = reshape(newPoints, [nNew, 1, 2]);%points的数据结构，新增加的point索引、帧索引、坐标            
            obj.live = [obj.live obj.nTrack + 1: obj.nTrack + nNew]; %其实就是1到nNew组成的一行向量
            obj.head = [obj.head; ones(nNew, 1) * frameIndex];%其实就是frameIndex这一帧有多少个nNew点，就标注成有多少个frameIndex,组成一列
            obj.tail = [obj.tail; ones(nNew, 1) * frameIndex];%同上
            obj.nTrack = obj.nTrack + nNew;%nTrack的意思是有多少个点
            obj.len = [obj.len; ones(nNew, 1)];%nTrack的长度？
        end
        
        function updatePoints(obj, uPoints, frameIndex)
            obj.points(obj.live, frameIndex, :)  = reshape(uPoints, [length(obj.live), 1, 2]);
            obj.len(obj.live) = obj.len(obj.live) + 1;%+1是为了记录update了多少次？
        end
        ...
        function [M2, id] = getM(obj, index)%这个函数的作用应该就是将在wSize范围内的帧的点，全部打包成一个窗口
            ...
            sub = sub(valid, :, :);
            id = valid;
            M2 = permute(sub, [3 1 2]);%置换维度，也就是sub的原本是point索引、帧索引、坐标 的顺序，换成 帧索引、坐标、point索引
        end
...
%             selected = obj.head <= frameIndex & obj.tail > frameIndex & (obj.labels(:, frameIndex, 1) ==1  | obj.labels(:, frameIndex, 2) == 1);
        ...
        function addLabel(obj, wSize)
            epsilon = 0.1:0.1:0.4;%[0.1 0.2 0.3 0.4]
            ...
            for windowIndex = 1:obj.nWindow
                ...
                computedLabels = find_best_segmentation(result, processedData, 100, epsilon);%应该是聚类出来的特征点，分类得到的标签
